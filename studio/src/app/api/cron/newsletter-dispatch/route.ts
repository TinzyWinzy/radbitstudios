...
function buildNewsletterHtml(title: string, body: string, link?: string): string {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://radbitstudios.co.zw';
  return `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#0a0a0a;color:#e5e0d8;margin:0;padding:40px 20px"><div style="max-width:600px;margin:0 auto;background:#111;border-radius:12px;padding:32px;border:1px solid #333"><h1 style="color:#1A8A7A;margin:0 0 16px;font-size:22px">${title}</h1><div style="color:#ccc;line-height:1.7;white-space:pre-wrap">${body.replace(/\n/g, '<br>')}</div>${link ? `<p style="margin-top:24px"><a href="${appUrl}${link}" style="display:inline-block;background:#1A8A7A;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600">View on Radbit</a></p>` : ''}<div style="margin-top:32px;padding-top:16px;border-top:1px solid #333;text-align:center"><p style="color:#666;font-size:12px;margin:0">Radbit — software tools for Zimbabwean enterprises</p><p style="color:#555;font-size:11px;margin:4px 0 8px">Harare, Zimbabwe</p><a href="${appUrl}/settings?tab=notifications" style="color:#999;font-size:11px;text-decoration:underline">Manage email preferences or unsubscribe</a></div></div></body></html>`;
}
