...
export async function generateTaxAnswer(input: TaxCopilotInput): Promise<TaxCopilotOutput> {
  ...
  const systemPrompt = `You are a Zimbabwean tax compliance assistant specializing in ZIMRA regulations for SMEs.
...
2. If the reference information does not contain enough detail, state that the user should consult a registered tax practitioner or contact ZIMRA directly.
3. List specific regulations or legal instruments that apply to the user's situation.
4. Include appropriate disclaimers about seeking professional advice.
